...
export function AppShell({ children }: PropsWithChildren) {
  ...
  return (
    <div className="relative">
      <div className="absolute inset-0 -z-10 surface-grid opacity-40" />
      <div className="shell-container grid min-w-0 gap-6 py-6 lg:grid-cols-[300px_minmax(0,1fr)]">
          ...
          <Card className="overflow-hidden p-4 lg:sticky lg:top-6">
            <div className="space-y-6">
              ...
              <div className="panel-subtle p-4">
                ...
                <div className="mt-2 font-display text-lg font-semibold text-foreground">{appConfig.workspace}</div>
                ...
                {navItems.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      href={item.href}
                      className={cn(
                        "flex min-w-0 items-center justify-between rounded-[1rem] border px-4 py-3 text-sm transition",
                        active
                          ? "border-accent/25 bg-accent/10 text-accent"
                          : "border-transparent text-muted-foreground hover:border-border/70 hover:bg-[hsl(var(--panel)/0.72)] hover:text-foreground dark:hover:border-white/10 dark:hover:bg-white/[0.05]"
                      )}
                    >
          ...
          <Card className="p-4">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div className="min-w-0">
                <div className="text-xs uppercase tracking-[0.24em] text-muted-foreground">
                  {pickLocale(locale, { ru: "Системный режим", en: "System mode" })}
                </div>
              ...
              <div className="flex w-full flex-wrap items-center gap-2 sm:gap-3 md:w-auto md:justify-end">
                <LanguageToggle />
                <ThemeToggle />
                <HeaderActionButton asChild variant="subtle" className="w-full min-[420px]:w-auto">
